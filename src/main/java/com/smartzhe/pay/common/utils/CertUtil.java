...
import static com.smartzhe.pay.common.utils.SDKUtil.isEmpty;
...
public class CertUtil {
    ...
    private static void initSignCert() {
        if (!"01".equals(MyAcpConfig.getInstance().getSignMethod())) {
            LogUtil.writeLog("非rsa签名方式，不加载签名证书。");
            return;
        }
        if (MyAcpConfig.getInstance().getSignCertPath() == null
                || MyAcpConfig.getInstance().getSignCertPwd() == null
                || MyAcpConfig.getInstance().getSignCertType() == null) {
            LogUtil.writeErrorLog("WARN: " + MyAcpConfig.SDK_SIGNCERT_PATH + "或" + MyAcpConfig.SDK_SIGNCERT_PWD
                    + "或" + MyAcpConfig.SDK_SIGNCERT_TYPE + "为空。 停止加载签名证书。");
            return;
        }
        if (null != keyStore) {
            keyStore = null;
        ...
        try {
            keyStore = getKeyInfo(MyAcpConfig.getInstance().getSignCertPath(),
                    MyAcpConfig.getInstance().getSignCertPwd(), MyAcpConfig
    ...
    private static void initMiddleCert() {
        LogUtil.writeLog("加载中级证书==>" + MyAcpConfig.getInstance().getMiddleCertPath());
        if (!isEmpty(MyAcpConfig.getInstance().getMiddleCertPath())) {
            middleCert = initCert(MyAcpConfig.getInstance().getMiddleCertPath());
    ...
    private static void initRootCert() {
        LogUtil.writeLog("加载根证书==>" + MyAcpConfig.getInstance().getRootCertPath());
        if (!isEmpty(MyAcpConfig.getInstance().getRootCertPath())) {
            rootCert = initCert(MyAcpConfig.getInstance().getRootCertPath());
    ...
    private static void initEncryptCert() {
        LogUtil.writeLog("加载敏感信息加密证书==>" + MyAcpConfig.getInstance().getEncryptCertPath());
        if (!isEmpty(MyAcpConfig.getInstance().getEncryptCertPath())) {
            encryptCert = initCert(MyAcpConfig.getInstance().getEncryptCertPath());
    ...
    private static void initTrackKey() {
        if (!isEmpty(MyAcpConfig.getInstance().getEncryptTrackKeyModulus())
                && !isEmpty(MyAcpConfig.getInstance().getEncryptTrackKeyExponent())) {
            encryptTrackKey = getPublicKey(MyAcpConfig.getInstance().getEncryptTrackKeyModulus(),
                    MyAcpConfig.getInstance().getEncryptTrackKeyExponent());
    ...
    private static void initValidateCertFromDir() {
        ...
        certMap.clear();
        String dir = MyAcpConfig.getInstance().getValidateCertDir();
    ...
    public static PrivateKey getSignCertPrivateKey() {
        try {
            ...
            PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
                    MyAcpConfig.getInstance().getSignCertPwd().toCharArray());
            return privateKey;
    ...
    public static PublicKey getEncryptCertPublicKey() {
        if (null == encryptCert) {
            String path = MyAcpConfig.getInstance().getEncryptCertPath();
            if (!isEmpty(path)) {
    ...
    private static KeyStore getKeyInfo(String pfxkeyfile, String keypwd,
        ...
        try {
            ...
            in = CertUtil.class.getResourceAsStream(pfxkeyfile);
            char[] nPassword = null;
    ...
    public static X509Certificate getMiddleCert() {
        if (null == middleCert) {
            String path = MyAcpConfig.getInstance().getMiddleCertPath();
            if (!isEmpty(path)) {
                initMiddleCert();
            } else {
                LogUtil.writeErrorLog(MyAcpConfig.SDK_MIDDLECERT_PATH + " not set in " + MyAcpConfig.FILE_NAME);
                return null;
            }
        }
        return middleCert;
    ...
    public static X509Certificate getRootCert() {
        if (null == rootCert) {
            String path = MyAcpConfig.getInstance().getRootCertPath();
            if (!isEmpty(path)) {
                initRootCert();
            } else {
                LogUtil.writeErrorLog(MyAcpConfig.SDK_ROOTCERT_PATH + " not set in " + MyAcpConfig.FILE_NAME);
                return null;
            }
        }
        return rootCert;
...
//			cert.verify(middleCert.getPublicKey());
        ...
        if (MyAcpConfig.getInstance().isIfValidateCNName()) {
            // 验证公钥是否属于银联
            if (!ACPConstants.UNIONPAY_CNNAME.equals(CertUtil.getIdentitiesFromCertficate(cert))) {
