...
import com.smartzhe.pay.common.config.MyAcpConfig;
...
public class SDKUtil {
	...
	public static boolean sign(Map<String, String> data, String encoding) {
		...
		String signMethod = data.get(ACPConstants.param_signMethod);
		String version = data.get(ACPConstants.param_version);
		if (!ACPConstants.VERSION_1_0_0.equals(version) && !ACPConstants.VERSION_5_0_1.equals(version) && isEmpty(signMethod)) {
		...
		if (ACPConstants.SIGNMETHOD_RSA.equals(signMethod)|| ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
			if (ACPConstants.VERSION_5_0_0.equals(version)|| ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
				// 设置签名证书序列号
				data.put(ACPConstants.param_certId, CertUtil.getSignCertId());
				...
				try {
					...
					// 设置签名域值
					data.put(ACPConstants.param_signature, stringSign);
					return true;
				} catch (Exception e) {
					LogUtil.writeErrorLog("Sign Error", e);
					return false;
				}
			} else if (ACPConstants.VERSION_5_1_0.equals(version)) {
				// 设置签名证书序列号
				data.put(ACPConstants.param_certId, CertUtil.getSignCertId());
				// 将Map信息转换成key1=value1&key2=value2的形式
				String stringData = coverMap2String(data);
		...
		} else if (ACPConstants.SIGNMETHOD_SHA256.equals(signMethod)) {
			return signBySecureKey(data, MyAcpConfig.getInstance()
					.getSecureKey(), encoding);
		} else if (ACPConstants.SIGNMETHOD_SM3.equals(signMethod)) {
			return signBySecureKey(data, MyAcpConfig.getInstance()
					.getSecureKey(), encoding);
		}
		return false;
	...
	public static boolean signBySecureKey(Map<String, String> data, String secureKey,
		...
		if (ACPConstants.SIGNMETHOD_SHA256.equals(signMethod)) {
			...
			// 设置签名域值
			data.put(ACPConstants.param_signature, strAfterSM3);
			return true;
		}
		return false;
	}
	...
	public static boolean signByCertInfo(Map<String, String> data,
		...
		if (ACPConstants.SIGNMETHOD_RSA.equals(signMethod) || ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
			if (ACPConstants.VERSION_5_0_0.equals(version) || ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
				// 设置签名证书序列号
				data.put(ACPConstants.param_certId, CertUtil.getCertIdByKeyStoreMap(certPath, certPwd));
				// 将Map信息转换成key1=value1&key2=value2的形式
				String stringData = coverMap2String(data);
				LogUtil.writeLog("待签名请求报文串:[" + stringData + "]");
				byte[] byteSign = null;
	...
	public static boolean validateBySecureKey(Map<String, String> resData, String secureKey, String encoding) {
		...
		String signMethod = resData.get(ACPConstants.param_signMethod);
		if (ACPConstants.SIGNMETHOD_SHA256.equals(signMethod)) {
			// 1.进行SHA256验证
			String stringSign = resData.get(ACPConstants.param_signature);
	...
	public static boolean validate(Map<String, String> resData, String encoding) {
		...
		String signMethod = resData.get(ACPConstants.param_signMethod);
		String version = resData.get(ACPConstants.param_version);
		if (ACPConstants.SIGNMETHOD_RSA.equals(signMethod) || ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
			// 获取返回报文的版本号
			if (ACPConstants.VERSION_5_0_0.equals(version) || ACPConstants.VERSION_1_0_0.equals(version) || ACPConstants.VERSION_5_0_1.equals(version)) {
				String stringSign = resData.get(ACPConstants.param_signature);
				...
				String certId = resData.get(ACPConstants.param_certId);
				...
				String strCert = resData.get(ACPConstants.param_signPubKeyCert);
//				LogUtil.writeLog("验签公钥证书：["+strCert+"]");
			...
			String strBeforeSha256 = stringData
					+ ACPConstants.AMPERSAND
					+ SecureUtil.sha256X16Str(MyAcpConfig.getInstance()
							.getSecureKey(), encoding);
			String strAfterSha256 = SecureUtil.sha256X16Str(strBeforeSha256,
					encoding);
			...
			String strBeforeSM3 = stringData
					+ ACPConstants.AMPERSAND
					+ SecureUtil.sm3X16Str(MyAcpConfig.getInstance()
							.getSecureKey(), encoding);
			String strAfterSM3 = SecureUtil
					.sm3X16Str(strBeforeSM3, encoding);
	...
	public static String coverMap2String(Map<String, String> data) {
		...
		while (it.hasNext()) {
			...
			if (ACPConstants.param_signature.equals(en.getKey().trim())) {
				continue;
			}
		...
		while (it.hasNext()) {
			Entry<String, String> en = it.next();
			sf.append(en.getKey() + ACPConstants.EQUAL + en.getValue()
					+ ACPConstants.AMPERSAND);
		}
	...
	public static int getEncryptCert(Map<String, String> resData,
			String encoding) {
		String strCert = resData.get(ACPConstants.param_encryptPubKeyCert);
		String certType = resData.get(ACPConstants.param_certType);
		if (isEmpty(strCert) || isEmpty(certType))
			return -1;
		X509Certificate x509Cert = CertUtil.genCertificateByStr(strCert);
		if (ACPConstants.CERTTYPE_01.equals(certType)) {
			// 更新敏感信息加密公钥
			if (!CertUtil.getEncryptCertId().equals(
					x509Cert.getSerialNumber().toString())) {
				// ID不同时进行本地证书更新操作
				String localCertPath = MyAcpConfig.getInstance().getEncryptCertPath();
		...
		} else if (ACPConstants.CERTTYPE_02.equals(certType)) {
//			// 更新磁道加密公钥
//			if (!CertUtil.getEncryptTrackCertId().equals(
//					x509Cert.getSerialNumber().toString())) {
//				// ID不同时进行本地证书更新操作
//				String localCertPath = MyAcpConfig.getConfig().getEncryptTrackCertPath();
//				String newLocalCertPath = genBackupName(localCertPath);
//				// 1.将本地证书进行备份存储
//				if (!copyFile(localCertPath, newLocalCertPath))
//					return -1;
...
//			}
	...
	public static String genBackupName(String fileName) {
		if (isEmpty(fileName))
			return "";
		int i = fileName.lastIndexOf(ACPConstants.POINT);
		String leftFileName = fileName.substring(0, i);
		String rightFileName = fileName.substring(i + 1);
		String newFileName = leftFileName + "_backup" + ACPConstants.POINT + rightFileName;
		return newFileName;
	}
