...
import com.smartzhe.pay.common.utils.ACPConstants;
import com.smartzhe.pay.common.utils.LogUtil;
import com.smartzhe.pay.common.utils.SDKUtil;
...
/**
 * 
 * @ClassName MyAcpConfig
 * @Description acpsdk配置文件acp_sdk.properties配置信息类
 * @date 2016-7-22 下午4:04:55
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class MyAcpConfig implements PayConfig {
	public static final String FILE_NAME = "acp.properties";
	/** 前台请求URL. */
	...
	public static final String SDK_ENCODING = "acpsdk.encoding";

	/** 操作对象. */
	private static MyAcpConfig instance = new MyAcpConfig();
	/** 属性文件对象. */
	 ...
	 * @return
	 */
	public static MyAcpConfig getInstance() {
		return instance;
	}
	...
	public void loadPropertiesFromSrc() {
		InputStream in = null;
		try {
			LogUtil.writeLog("从classpath: " +MyAcpConfig.class.getClassLoader().getResource("").getPath()+" 获取属性文件"+FILE_NAME);
			in = MyAcpConfig.class.getClassLoader().getResourceAsStream(FILE_NAME);
			if (null != in) {
				properties = new Properties();
				try {
					properties.load(in);
				...
				LogUtil.writeErrorLog(FILE_NAME + "属性文件未能在classpath指定的目录下 "+MyAcpConfig.class.getClassLoader().getResource("").getPath()+" 找到!");
				return;
			}
			loadProperties(properties);
		} catch (IOException e) {
	...
	public void loadProperties(Properties pro) {
		...
		if (!SDKUtil.isEmpty(value)) {
			if( ACPConstants.FALSE_STRING.equals(value.trim()))
					this.ifValidateCNName = false;
		}
		
		value = pro.getProperty(SDK_IF_VALIDATE_REMOTE_CERT);
		if (!SDKUtil.isEmpty(value)) {
			if( ACPConstants.TRUE_STRING.equals(value.trim()))
		...
		value = pro.getProperty(SDK_ENCODING);
		if (!SDKUtil.isEmpty(value)) {
